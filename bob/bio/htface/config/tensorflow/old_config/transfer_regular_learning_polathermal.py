...
directory = "/idiap/temp/tpereira/casia_webface/polathermal/resnet_gray_siamese_transfer_2/"
checkpoint_filename = "/idiap/temp/tpereira/casia_webface/official_checkpoints/160x/resnet_inception_gray_centerloss/"


def build_graph(inputs, reuse=False, is_training=True):
    prelogits, _ = inception_resnet_v2(inputs, reuse=reuse, is_training=True)
    return prelogits
...
if from_checkpoint:
    ...
    graph['left'] = build_graph(inputs['left'], is_training=True)
    ...
    graph['right'] = build_graph(inputs['right'], reuse=True, is_training=True)
