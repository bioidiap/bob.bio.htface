...
model_filename = inception_resnet_v1_casia_webface_gray_batch_norm # Value wired by path


inputs = tf.placeholder(tf.float32, shape=(1, 160, 160, 1))

# Taking the embedding
prelogits, end_points = inception_resnet_v1_batch_norm(tf.stack([tf.image.per_image_standardization(i) for i in tf.unstack(inputs)]),
